          ...
          <div className="space-y-6">
            ...
            {config.sectors.length === 0 ? (
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {/* Columna izquierda: Sectores, configuración global y personalización visual */}
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3 max-h-96 overflow-y-auto">
                      {config.sectors.map((sector, index) => (
                        <div key={sector.id} className="bg-white/5 rounded-lg border border-white/10 overflow-hidden">
                          {/* Header del sector */}
                          <div className={`p-4 cursor-pointer transition-all hover:bg-white/10 ${selectedSectorId === sector.id ? 'bg-purple-500/20 border-b border-white/10' : ''}`}
                            onClick={() => setSelectedSectorId(selectedSectorId === sector.id ? null : sector.id)}>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-3">
                                <div className="w-6 h-6 rounded-full border-2 border-white/30" style={{ backgroundColor: sector.backgroundColor }}></div>
                                <div>
                                  <div className="text-white font-medium">{sector.displayName || sector.formalName}</div>
                                  <div className="text-gray-400 text-sm">{sector.isPrize ? 'Premio' : 'No premio'}</div>
                                </div>
                              </div>
                              <div className="flex items-center space-x-2">
                                <button onClick={e => { e.stopPropagation(); duplicateSector(sector.id); }} className="p-2 text-gray-400 hover:text-white transition-colors rounded" title="Duplicar">📋</button>
                                <button onClick={e => { e.stopPropagation(); removeSector(sector.id); }} className="p-2 text-gray-400 hover:text-red-400 transition-colors rounded" title="Eliminar">🗑️</button>
                                <div className={`transition-transform ${selectedSectorId === sector.id ? 'rotate-180' : ''}`}>▼</div>
                              </div>
                            </div>
                          </div>
                          ...
                          {selectedSectorId === sector.id && (
                            <div className="p-4 bg-white/5 border-t border-white/10">
                              {/* Aquí va la configuración editable del sector (nombre, color, probabilidad, etc.) */}
                              {/* ...puedes restaurar la lógica previa aquí... */}
                            </div>
                          )}
                        </div>
                      ))}
                  ...
                  <div className="bg-white/5 rounded-lg p-6 border border-white/10">
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Velocidad de Giro</label>
                        <select className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-purple-500 focus:outline-none">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-300 mb-2">Sonidos</label>
                        <label className="flex items-center space-x-3 cursor-pointer">
                          <input type="checkbox" defaultChecked className="w-4 h-4 text-purple-600 bg-white/10 border-white/20 rounded focus:ring-purple-500 focus:ring-2" />
                          <span className="text-white">Activar sonidos de giro</span>
                        </label>
                      </div>
                    </div>
                  </div>
                  {/* Personalización Visual (imágenes y sliders) */}
                  <div className="bg-white/5 rounded-lg p-6 border border-white/10">
                    ...
                    <div className="space-y-4">
                      ...
                      <input type="file" accept="image/*" onChange={e => { const file = e.target.files?.[0]; if (file) { setConfig(prev => ({ ...prev, backgroundImage: URL.createObjectURL(file) })); } }} className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-purple-500 focus:outline-none" />
                      <label className="block text-sm font-medium text-gray-300 mb-2">Borde de la Ruleta (PNG)</label>
                      <input type="file" accept="image/png" onChange={e => { const file = e.target.files?.[0]; if (file) { setConfig(prev => ({ ...prev, borderImage: URL.createObjectURL(file) })); } }} className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-purple-500 focus:outline-none" />
                      <label className="block text-sm font-medium text-gray-300 mb-2">Marcador Central (PNG)</label>
                      <input type="file" accept="image/png" onChange={e => { const file = e.target.files?.[0]; if (file) { setConfig(prev => ({ ...prev, centerMarkerImage: URL.createObjectURL(file) })); } }} className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:border-purple-500 focus:outline-none" />
                      <label className="block text-sm font-medium text-gray-300 mb-2">Tamaño del Borde</label>
                      <input type="range" min="0" max="4" step="0.1" value={config.borderSize || 0} onChange={e => setConfig(prev => ({ ...prev, borderSize: parseFloat(e.target.value) }))} className="w-full" />
                      <label className="block text-sm font-medium text-gray-300 mb-2">Tamaño del Marcador Central</label>
                      <input type="range" min="0" max="8" step="0.1" value={config.centerMarkerSize || 0} onChange={e => setConfig(prev => ({ ...prev, centerMarkerSize: parseFloat(e.target.value) }))} className="w-full" />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-white/5 rounded-lg p-6 border border-white/10">
                    <div className="aspect-w-9 aspect-h-16 bg-black relative">
                      {config.backgroundImage && (
                        <img src={config.backgroundImage} alt="Fondo" className="absolute inset-0 w-full h-full object-cover" />
                      )}
                      <RouletteWheel
