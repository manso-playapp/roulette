...
export const RouletteWheel: React.FC<RouletteWheelProps> = ({
  ...
  useEffect(() => {
    ...
    for (let index = 0; index < activeSectors.length; index++) {
      // Clonar profundamente el sector para evitar mutaciones accidentales
      const sector = JSON.parse(JSON.stringify(activeSectors[index]));
      // Log temporal para depuración
      // eslint-disable-next-line no-console
      console.log(`Render sector #${index}:`, sector.displayName, sector.backgroundColor, sector.order);
      ...
      if (sector.curvedText) {
        ...
        ctx.font = `${sector.fontSize ?? 16}px ${fontFamily}`;
