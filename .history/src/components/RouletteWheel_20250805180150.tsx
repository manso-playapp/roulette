...
export const RouletteWheel: React.FC<RouletteWheelProps> = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < orderedSectors.length; i++) {
      ...
      // Usar 60% del radio para mejor visibilidad
      const textRadius = radius * 0.6;
      const textX = centerX + Math.cos(midAngle) * textRadius;
      const textY = centerY + Math.sin(midAngle) * textRadius;
      ctx.save();
      ctx.fillStyle = sector.textColor;
      const fontPx = Math.max(10, Math.min(sector.fontSize ?? 16, 24));
      ctx.font = `${fontPx}px ${sector.fontFamily || 'Arial, sans-serif'}`;
      ...
      for (let lineIndex = 0; lineIndex < textLines.length; lineIndex++) {
        ...
        ctx.save();
        // Mover al punto de texto
        ctx.translate(textX, textY + yOffset);
        
        // Rotar para que el texto sea perpendicular al radio
        // Ajustar la rotación según la posición del sector
        let textRotation = midAngle + Math.PI / 2;
        
        // Si el texto estaría boca abajo (entre 90° y 270°), rotarlo 180° más para legibilidad
        // Pero ajustar el rango para que todos los textos apunten hacia el centro
        ...
        let renderLine = line;
        // Calcular ancho máximo basado en el ángulo del sector
        const sectorAngle = endAngle - startAngle;
        const maxTextWidth = Math.min(radius * 0.8, (textRadius * sectorAngle) * 0.8);
