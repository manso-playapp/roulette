  ...
  useEffect(() => {
    ...
    for (let i = 0; i < orderedSectors.length; i++) {
      ...
      for (let lineIndex = 0; lineIndex < textLines.length; lineIndex++) {
        ...
        // Rotar para que el texto sea perpendicular al radio (tangente a la circunferencia)
        ctx.rotate(midAngle + Math.PI / 2);
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        let renderLine = line;
        // Recortar si el texto es muy largo para el sector
        const maxTextWidth = radius * 0.7;
        if (ctx.measureText(renderLine).width > maxTextWidth) {
          while (renderLine.length > 0 && ctx.measureText(renderLine + '...').width > maxTextWidth) {
