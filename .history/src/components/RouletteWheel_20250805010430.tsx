...
export const RouletteWheel: React.FC<RouletteWheelProps> = ({
  ...
  useEffect(() => {
    ...
    activeSectors.forEach((sector, index) => {
      ...
      // Configurar fuente y centrado
      ctx.fillStyle = sector.textColor;
      const fontFamily = sector.fontFamily || 'Roboto, Arial, sans-serif';
      ...
      textLines.forEach((line, lineIndex) => {
        ...
        // SIEMPRE usar el centrado automático del canvas para garantizar centrado perfecto
        // Solo aplicar espaciado de letras como estilo CSS si está disponible
        const letterSpacing = sector.letterSpacing ?? 0;
        
        if (Math.abs(letterSpacing) > 0.1 && 'letterSpacing' in ctx.font) {
          // Intentar usar letter-spacing nativo del canvas si está disponible
        ...
        } else if (Math.abs(letterSpacing) > 0.1) {
          // Fallback: espaciado manual pero manteniendo centrado perfecto
          ctx.save();
          
          // Medir el texto original para calcular el centro real
          const originalWidth = ctx.measureText(line).width;
          ...
          // Empezar desde el centro calculado
          let x = -totalWidth / 2;
          
          // Dibujar cada carácter manteniendo el centrado
          chars.forEach(char => {
          ...
          // Caso normal: centrado perfecto automático
          ctx.fillText(line, 0, yOffset);
        }
      });
