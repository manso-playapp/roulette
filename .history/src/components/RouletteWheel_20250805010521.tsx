  ...
  useEffect(() => {
    ...
    activeSectors.forEach((sector, index) => {
      ...
      textLines.forEach((line, lineIndex) => {
        ...
        // SIMPLIFICADO: Siempre usar centrado automático del canvas
        // El textAlign='center' garantiza centrado perfecto en todas las situaciones
        const letterSpacing = sector.letterSpacing ?? 0;
        
        if (Math.abs(letterSpacing) > 0.1) {
          // Para espaciado de letras, usar aproximación manual pero centrada
          const chars = line.split('');
          
          // Calcular ancho real del texto con espaciado
          let totalWidth = 0;
          chars.forEach(char => {
          ...
          chars.forEach(char => {
            ...
            ctx.fillText(char, currentX + charWidth/2, yOffset);
            currentX += charWidth + letterSpacing;
          });
        } else {
          // Caso estándar: centrado automático perfecto
