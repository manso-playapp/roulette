...
export const RouletteWheel: React.FC<RouletteWheelProps> = ({
  ...
  useEffect(() => {
    ...
    activeSectors.forEach((sector, index) => {
      ...
      const fontFamily = sector.fontFamily || 'Roboto';
      ctx.font = `${sector.fontSize}px ${fontFamily}`;
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';

      // Procesar texto: automáticamente convertir comas en saltos de línea
      const displayText = sector.displayName || `SECTOR ${index + 1}`;
      const textLines = displayText.includes(',') 
        ? displayText.split(',').map(line => line.trim()).filter(line => line.length > 0)
        : [displayText];
      ...
      textLines.forEach((line, lineIndex) => {
        ...
        // PROFESIONAL: Solo usar centrado automático del canvas
        // Aplicar letter-spacing solo si es necesario y de forma sutil
        const letterSpacing = sector.letterSpacing ?? 0;
        
        if (letterSpacing !== 0) {
          // Usar CSS letter-spacing nativo si es posible
          const originalFont = ctx.font;
          ctx.font = `${sector.fontSize}px ${fontFamily}`;
          // Note: Canvas no soporta letter-spacing nativo, pero mantenemos simple
        }
