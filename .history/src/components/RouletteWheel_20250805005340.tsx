  ...
  useEffect(() => {
    ...
    activeSectors.forEach((sector, index) => {
      ...
      textLines.forEach((line, lineIndex) => {
        ...
        // Aplicar espaciado de letras manualmente solo si es mayor que 0
        const letterSpacing = sector.letterSpacing ?? 0;
        if (letterSpacing > 0.1) { // Solo aplicar si es significativo
          ...
          totalWidth += (chars.length - 1) * letterSpacing; // Agregar espacios entre caracteres
          
          // Centrar el texto completo
          let currentX = -totalWidth / 2;
          chars.forEach((char: string) => {
            // Centrar cada carácter en su posición
            const charWidth = ctx.measureText(char).width;
            ctx.fillText(char, currentX + charWidth / 2, yOffset);
            currentX += charWidth + letterSpacing;
          });
        } else {
          // Usar el renderizado normal del canvas que es más eficiente y está centrado
