...
import { MinimalFontSelect } from '../../components/MinimalFontSelect';
                            ...
                            <div className="p-3 bg-white/5 border-t border-white/10">
                              <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-2 mt-2">
                                  <div>
                                    <label className="block text-xs font-medium text-gray-300 mb-1">Fuente</label>
                                    <MinimalFontSelect
                                      value={sector.fontFamily || 'Roboto'}
                                      onValueChange={(v: string) => updateSector(sector.id, { fontFamily: v })}
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-xs font-medium text-gray-300 mb-1">Tamaño <span className='text-white/50'>({sector.fontSize || 16}px)</span></label>
                                    <MinimalSlider
                                      min={10}
                                      max={64}
                                      step={1}
                                      value={sector.fontSize || 16}
                                      onValueChange={(v: number) => updateSector(sector.id, { fontSize: v })}
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-xs font-medium text-gray-300 mb-1">Interlineado <span className='text-white/50'>({sector.lineHeight || 1.2})</span></label>
                                    <MinimalSlider
                                      min={0.8}
                                      max={2.5}
                                      step={0.01}
                                      value={sector.lineHeight || 1.2}
                                      onValueChange={(v: number) => updateSector(sector.id, { lineHeight: v })}
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-xs font-medium text-gray-300 mb-1">Interletrado <span className='text-white/50'>({sector.letterSpacing || 0}px)</span></label>
                                    <MinimalSlider
                                      min={-2}
                                      max={10}
                                      step={0.1}
                                      value={sector.letterSpacing || 0}
                                      onValueChange={(v: number) => updateSector(sector.id, { letterSpacing: v })}
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-xs font-medium text-gray-300 mb-1">Espaciado entre líneas <span className='text-white/50'>({sector.interlineSpacing || 0})</span></label>
                                    <MinimalSlider
                                      min={0}
                                      max={10}
                                      step={0.1}
                                      value={sector.interlineSpacing || 0}
                                      onValueChange={(v: number) => updateSector(sector.id, { interlineSpacing: v })}
                                    />
                                  </div>
                                  <div>
                                    <label className="block text-xs font-medium text-gray-300 mb-1">Interletrado línea 2 <span className='text-white/50'>({sector.interletterSpacingLine2 || 0})</span></label>
                                    <MinimalSlider
                                      min={-2}
                                      max={10}
                                      step={0.1}
                                      value={sector.interletterSpacingLine2 || 0}
                                      onValueChange={(v: number) => updateSector(sector.id, { interletterSpacingLine2: v })}
                                    />
                                  </div>
                                  <div className="flex items-center space-x-2 mt-1">
                                    <input
                                      type="checkbox"
                                      checked={!!sector.curvedText}
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-gray-300 mb-1">Distancia desde el centro <span className='text-white/50'>({sector.distanceFromCenter || 70})</span></label>
                                    <MinimalSlider
                                      min={30}
                                      max={100}
                                      step={1}
                                      value={sector.distanceFromCenter || 70}
                                      onValueChange={(v: number) => updateSector(sector.id, { distanceFromCenter: v })}
                                    />
                                  </div>
                                </div>
