    ...
    activeSectors.forEach((sector, index) => {
      ...
      textLines.forEach((line, lineIndex) => {
        ...
        if (Math.abs(letterSpacing) > 0.1) { // Aplicar para valores positivos o negativos significativos
          ...
          // Calcular ancho total de los caracteres sin espaciado
          let charsWidth = 0;
          chars.forEach((char: string) => {
            charsWidth += ctx.measureText(char).width;
          });
          
          // El ancho total incluye el espaciado entre caracteres
          const totalSpacing = (chars.length - 1) * letterSpacing;
          const totalWidth = charsWidth + totalSpacing;
          
          // Empezar desde la posición que centre todo el texto
