  ...
  useEffect(() => {
    ...
    activeSectors.forEach((sector, index) => {
      ...
      textLines.forEach((line, lineIndex) => {
        ...
        // Verificar si hay espaciado de letras personalizado
        const letterSpacing = sector.letterSpacing ?? 0;
        
        if (Math.abs(letterSpacing) > 0.1) {
          // Aplicar espaciado manual manteniendo el centrado
          const chars = line.split('');
          
          // Calcular el ancho total del texto con espaciado
          let totalTextWidth = 0;
          chars.forEach((char: string) => {
            totalTextWidth += ctx.measureText(char).width;
          });
          totalTextWidth += (chars.length - 1) * letterSpacing;
          
          // Empezar desde la posición que centre el texto completo
          let currentX = -totalTextWidth / 2;
          
          // Dibujar cada carácter con su espaciado
          chars.forEach((char: string) => {
            ctx.fillText(char, currentX, yOffset);
            currentX += ctx.measureText(char).width + letterSpacing;
          });
        } else {
          // Sin espaciado personalizado, usar centrado automático del canvas
