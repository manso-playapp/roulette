  ...
  useEffect(() => {
    ...
    for (let i = 0; i < orderedSectors.length; i++) {
      ...
      for (let lineIndex = 0; lineIndex < textLines.length; lineIndex++) {
        ...
        const letterSpacing = lineIndex === 1 && sector.interletterSpacingLine2 !== undefined
          ? sector.interletterSpacingLine2
          : sector.letterSpacing ?? 0;

        if (sector.curvedText) {
          // Texto curvo: dibujar cada letra siguiendo la curvatura
          const chars = line.split('');
          // Ángulo total ocupado por la línea
          const textLength = chars.length;
          // Ajustar el radio para el texto curvo
          const curvedRadius = textRadius;
          // Ángulo por letra (aprox)
          const anglePerChar = (letterSpacing > 0 ? (fontPx + letterSpacing) : fontPx) / (2 * Math.PI * curvedRadius) * 2 * Math.PI;
          // Centrar el texto curvo
          const startAngle = -((textLength - 1) / 2) * anglePerChar;
          for (let c = 0; c < textLength; c++) {
            const char = chars[c];
            ctx.save();
            ctx.rotate(startAngle + c * anglePerChar);
