...
interface RouletteWheelProps {
  ...
  borderSize?: number; // Tamaño del borde
  centerMarkerImage?: string; // URL del marcador central personalizado
  centerMarkerSize?: number; // Tamaño del marcador central
}
...
export const RouletteWheel: React.FC<RouletteWheelProps> = ({
  ...
  useEffect(() => {
    ...
    activeSectors.forEach((sector, index) => {
      ...
      ctx.font = `${sector.fontSize}px "${sector.fontFamily}", sans-serif`;
      ...
      textLines.forEach((line, lineIndex) => {
        ...
        // Aplicar espaciado de letras manualmente si es necesario
        const letterSpacing = sector.letterSpacing ?? 0;
        if (letterSpacing > 0) {
          const chars = line.split('');
          const totalWidth = chars.reduce((sum: number, char: string) => 
            sum + ctx.measureText(char).width + letterSpacing, 0
          ) - letterSpacing;
      ...
      // Mostrar probabilidad en modo debug
    ...
    // Dibujar borde personalizado si existe
    if (borderImage) {
      const borderImg = new Image();
      borderImg.src = borderImage;
      borderImg.onload = () => {
        ctx.drawImage(
          borderImg,
          centerX - radius - borderSize,
          centerY - radius - borderSize,
          (radius + borderSize) * 2,
          (radius + borderSize) * 2
        );
      };
    ...
    // Dibujar marcador central personalizado si existe
    if (centerMarkerImage) {
      const markerImg = new Image();
      markerImg.src = centerMarkerImage;
      markerImg.onload = () => {
        ctx.drawImage(
          markerImg,
          centerX - centerMarkerSize / 2,
          centerY - centerMarkerSize / 2,
          centerMarkerSize,
          centerMarkerSize
        );
      };
  ...
  }, [activeSectors, animation.currentAngle, width, height, borderImage, borderSize, centerMarkerImage, centerMarkerSize]);
