...
export const RouletteWheel: React.FC<RouletteWheelProps> = ({
  ...
  useEffect(() => {
    ...
    // Renderizado limpio: cada sector usa su propio ángulo basado en el índice
    for (let i = 0; i < activeSectors.length; i++) {
      const sector = activeSectors[i];
      const startAngle = (i / totalSectors) * 2 * Math.PI - Math.PI / 2;
      const endAngle = ((i + 1) / totalSectors) * 2 * Math.PI - Math.PI / 2;
      ...
      // Dibujar texto centrado en el sector
      ...
      const textLines = (sector.textLines && sector.textLines.length > 0)
        ? sector.textLines
        : [sector.displayName || `SECTOR ${i + 1}`];
