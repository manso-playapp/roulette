...
export const useRouletteConfig = () => {
    ...
    setConfig(prev => ({
      ...
      sectors: prev.sectors.map(sector => {
        if (sector.id === sectorId) {
          ...
          // Si se actualiza displayName, procesar automáticamente las comas para saltos de línea
          if (updates.displayName !== undefined) {
            const lines = updates.displayName
              .split(',')
              .map(line => line.trim())
              .filter(line => line.length > 0);
            updatedSector.textLines = lines;
          }
  ...
  // Centrar todos los textos (resetear interletrado a 0)
  const centerAllTexts = useCallback(() => {
    setConfig(prev => ({
      ...prev,
      sectors: prev.sectors.map(sector => ({
        ...sector,
        letterSpacing: 0, // Resetear para centrado perfecto
        fontFamily: sector.fontFamily || 'Roboto'
      })),
      updatedAt: new Date()
    }));
  }, []);

  // Procesar automáticamente todas las comas en saltos de línea
  const processAllCommas = useCallback(() => {
    setConfig(prev => ({
      ...prev,
      sectors: prev.sectors.map(sector => {
        if (sector.displayName.includes(',')) {
          const lines = sector.displayName
            .split(',')
            .map(line => line.trim())
            .filter(line => line.length > 0)
